...
package Bio::KBase::SSHAgent::Key;
use strict;
...
sub new_from_blob {
    my $class = shift;
    my($blob) = @_;
    my $b = Bio::KBase::SSHAgent::Buffer->new( MP => 'SSH1' );
    $b->append($blob);
...
Bio::KBase::SSHAgent::Key - Public or private key abstraction

=head1 SYNOPSIS

    use Bio::KBase::SSHAgent::Key;
    my $key = Bio::KBase::SSHAgent::Key->new;

=head1 DESCRIPTION

I<Bio::KBase::SSHAgent::Key> implements an abstract base class interface
...
=head2 Bio::KBase::SSHAgent::Key->new($key_type [, $blob [, $compat_flag_ref ]])

Creates a new object of type I<Bio::KBase::SSHAgent::Key::$key_type>,
...
flag, which is generally stored inside of the I<Bio::KBase::SSHAgent>
...
=head2 Bio::KBase::SSHAgent::Key->read_private($key_type, $file [, $pass])
...
=head2 Bio::KBase::SSHAgent::Key->keygen($key_type, $bits)
...
=head2 Bio::KBase::SSHAgent::Key->extract_public($key_type, $key_string)
