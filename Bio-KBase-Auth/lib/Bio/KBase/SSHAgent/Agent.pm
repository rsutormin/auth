...
package Bio::KBase::SSHAgent::Agent;
use strict;

use IO::Socket;
use Carp qw( croak );
use Bio::KBase::SSHAgent::Constants qw( :agent SSH_COM_AGENT2_FAILURE );
use Bio::KBase::SSHAgent::Buffer;
...
sub request {
    ...
    $buf = Bio::KBase::SSHAgent::Buffer->new( MP => "SSH$agent->{version}" );
...
Bio::KBase::SSHAgent::Agent - Client for agent authentication

=head1 SYNOPSIS

    use Bio::KBase::SSHAgent::Agent;
    my $agent = Bio::KBase::SSHAgent::Agent->new(2);  ## SSH-2 protocol
...
I<Bio::KBase::SSHAgent::Agent> provides a client for agent-based
...
which the SSH client (ie. I<Bio::KBase::SSHAgent::>, in this case) can
...
I<Bio::KBase::SSHAgent::Agent> implements the client portion of the
authentication agent; this is the piece that interfaces with
I<Bio::KBase::SSHAgent::>'s authentication mechanism to contact the
...
succeeds, I<Bio::KBase::SSHAgent::> will try to use the identities
returned from the agent, in addition to any identity files on
disk.

=head1 USAGE

=head2 Bio::KBase::SSHAgent::Agent->new($version)
...
the next key (a subclass of I<Bio::KBase::SSHAgent::Key>). If called
...
where I<$ssh> is a I<Bio::KBase::SSHAgent::SSH1> object.

This method is only applicable in SSH-1.

=head1 AUTHOR & COPYRIGHTS

Please see the Bio::KBase::SSHAgent:: manpage for author, copyright,
